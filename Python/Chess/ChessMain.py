...
def loadImages():
    pieces = ["bP", "bQ", "bK", "bB", "bN", "bR",
              "wP", "wQ", "wK", "wB", "wN", "wR",
              "bPt", "bQt", "bKt", "bBt", "bNt",
              "bRt", "wPt", "wQt", "wKt", "wBt", "wNt", "wRt"]
    for piece in pieces:
        IMAGES[piece] = p.transform.smoothscale(
            p.image.load("Python/Chess/Images/" + piece + ".png"),
            (SQ_SIZE, SQ_SIZE))
...
def DrawState(screen, board, selected_piece, moves):
    ...
    if selected_piece is not None:
        for m in moves:
            ...
            p.draw.circle(screen, p.Color('green'),
                          ((c1+0.5)*SQ_SIZE, (r1+0.5)*SQ_SIZE), 5)
    ...
    while True:
        ...
        for e in p.event.get():
            ...
            if e.type == p.MOUSEBUTTONDOWN:
                if piece != '--' and selected_piece is None:
            ...
            if e.type == p.MOUSEBUTTONUP and selected_piece is not None:
                if promote:
                    promote = KeyPromote()
